...
export async function verifyRazorpayPayment({
  ...
  const isDemo = keySecret === 'demo_secret_key_abcdef' || razorpayOrderId.startsWith('order_cp_') || !razorpaySignature;
  ...
  if (simulateFailure) {
    await db.addAuditLog({
      ...
      detail: 'Payment authorization declined by issuer. Cart has been preserved with zero duplicate charge.',
      agent: 'ORCHESTRATOR',
      payload: {
    ...
    return {
      success: false,
      message: 'Payment could not be completed. Your cart has been preserved and no charge was made.',
      orderId: razorpayOrderId,
      paymentId: razorpayPaymentId,
      cartPreserved: true
    };
