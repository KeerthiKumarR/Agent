import { PolicyCheckResult, PolicyStatus, Cart, Customer, MerchantPolicy } from '../types';
import { db } from '../db';

export interface PolicyEvaluationInput {
  actionType: 'SEND_CAMPAIGN' | 'APPLY_DISCOUNT' | 'INITIATE_PAYMENT' | 'MODIFY_CART';
  cart?: Cart;
  customer?: Customer;
...
export const policyEngine = {
   ...
   */
  evaluate: async (input: PolicyEvaluationInput): Promise<PolicyCheckResult> => {
    ...
    if (input.actionType === 'INITIATE_PAYMENT') {
      if (policies.requirePaymentApproval) {
        return {
          ...
          reason: 'Trust Boundary: AI agent cannot directly charge payment without explicit customer interaction and approval.',
          limits: {
    ...
    // Rule 5: Discount Cap
    if (input.proposedDiscount && input.proposedDiscount > policies.maxDiscountPercent) {
      return {
        status: 'BLOCKED',
