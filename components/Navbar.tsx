  ...
  const fetchCart = async () => {
    try {
      ...
      if (res.ok) {
        ...
        const totalItems = data.cart?.items?.reduce((sum: number, item: any) => sum + item.quantity, 0) || 0;
        ...
        <nav className="hidden md:flex items-center space-x-1 lg:space-x-1.5">
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                <Icon className={`w-3.5 h-3.5 ${isActive ? 'text-primary-light' : 'text-text-muted'}`} />
                <span>{item.name}</span>
