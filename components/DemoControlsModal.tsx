...
export default function DemoControlsModal() {
  ...
  const handleAddProduct = async () => {
    ...
    try {
      ...
      if (res.ok) {
        notify('Added AeroRun Waterproof Shoes to cart! Growth Agent will detect upsell.');
        router.push('/cart');
      }
    } catch (e) {
  ...
  const handleSimulateAbandonment = async () => {
    ...
    try {
      await fetch('/api/demo', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'SIMULATE_ABANDONMENT', payload: { inactivityMinutes: 75 } })
      });
  ...
  const handleSimulatePaymentFailure = async () => {
    ...
    try {
      const createRes = await fetch('/api/payment/create-order', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ cartId: 'cart_abandoned_01', amount: 4999, simulateFailure: true })
      });
      notify('Simulating payment decline on checkout page with cart preservation guarantee.');
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-text-muted">
                    Trigger live agent workflows, edge cases, and autonomous commerce loops.
                  </p>
                </div>
              </div>
              <button
              ...
              <div className="space-y-2.5">
                ...
                <div className="p-3.5 rounded-xl bg-surface border border-border/80 hover:border-primary/50 transition-colors flex items-center justify-between gap-3">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <div className="text-xs font-bold text-white">1. Add AeroRun Shoes + Trigger Growth Upsell</div>
                      <div className="text-[11px] text-text-muted">Adds shoes to cart & activates Growth Agent to recommend Performance Socks (96% match).</div>
                    </div>
                  </div>
                  <button
                ...
                <div className="p-3.5 rounded-xl bg-surface border border-border/80 hover:border-amber-500/50 transition-colors flex items-center justify-between gap-3">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <div className="text-xs font-bold text-white">2. Simulate Cart Abandonment</div>
                      <div className="text-[11px] text-text-muted">Simulates 75 mins inactivity, calculates 87% intent score, and logs audit observation.</div>
                    </div>
                  </div>
                  <button
                ...
                <div className="p-3.5 rounded-xl bg-surface border border-border/80 hover:border-cyan-500/50 transition-colors flex items-center justify-between gap-3">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <div className="text-xs font-bold text-white">3. Execute Autonomous Agent Recovery</div>
                      <div className="text-[11px] text-text-muted">Runs Observe → Reason → Policy Check → Act → Verify → Learn loop & creates WhatsApp message.</div>
                    </div>
                  </div>
                  <button
                ...
                <div className="p-3.5 rounded-xl bg-surface border border-border/80 hover:border-rose-500/50 transition-colors flex items-center justify-between gap-3">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <div className="text-xs font-bold text-white">4. Simulate Payment Failure & Safe Retry</div>
                      <div className="text-[11px] text-text-muted">Demonstrates safe failure handling: cart preserved, zero double-charge, safe retry button.</div>
                    </div>
                  </div>
                  <button
                  ...
                  >
                    {loadingAction === 'pay_fail' ? 'Loading...' : 'Simulate Fail'}
                  </button>
                </div>
              </div>
