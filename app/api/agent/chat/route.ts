...
export async function POST(req: NextRequest) {
  try {
    ...
    const explanation = await commerceAgent.explainRecommendation(intent, rankedMatches);
    ...
    await db.addAuditLog({
      ...
      payload: { query: message, intent, topMatch: rankedMatches[0]?.product.name, matchScore: rankedMatches[0]?.matchScore }
    });
