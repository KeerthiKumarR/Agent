...
export async function POST(req: NextRequest) {
  try {
    const { productId, quantity = 1, isUpsell = false, cartId } = await req.json();
    if (!productId) {
      return NextResponse.json({ error: 'productId is required' }, { status: 400 });
    }

    const activeCart = cartId ? await db.getCartById(cartId) : await db.getActiveCart();
    const targetCartId = activeCart?.id || 'cart_active_user';

    const updatedCart = await db.addToCart(targetCartId, productId, quantity, isUpsell);
    ...
    await db.addAuditLog({
      ...
      payload: { productId, price: product?.price, quantity, isUpsell, newTotal: updatedCart.total }
    });
