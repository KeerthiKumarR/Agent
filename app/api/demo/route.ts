    ...
    switch (action) {
      case 'RESET_ALL': {
        ...
        await db.addAuditLog({
          ...
          detail: 'Demo database and memory state reset to initial Velocity Sports benchmark state.',
          agent: 'ORCHESTRATOR'
        });
        return NextResponse.json({ success: true, message: 'Store reset to initial state' });
      }
      ...
      case 'SIMULATE_ABANDONMENT': {
        ...
        await db.updateCartStatus(cart.id, 'ABANDONED', payload?.inactivityMinutes || 75);
        await db.addAuditLog({
          type: 'EVENT',
          ...
          agent: 'ORCHESTRATOR',
          payload: { cartId: cart.id, inactivityMinutes: payload?.inactivityMinutes || 75 }
        });
      ...
      default:
        return NextResponse.json({ error: 'Unknown demo simulation action' }, { status: 400 });
    }
  } catch (error: any) {
